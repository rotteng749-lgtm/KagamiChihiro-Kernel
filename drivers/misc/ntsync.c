...
static void dev_lock_obj(struct ntsync_device *dev, struct ntsync_obj *obj)
{
	...
	lockdep_assert_held(obj->dev == dev);
	spin_lock(&obj->lock);
	/*
...
static void obj_lock(struct ntsync_obj *obj)
{
	...
	for (;;) {
		 ...
		 * be clear.
		 */
		lockdep_assert_held(!obj->dev_locked);
		spin_unlock(&obj->lock);
			 ...
			 struct ntsync_obj *locked_obj)
{
	...
	lockdep_assert_held(&dev->wait_all_lock);
	if (locked_obj)
		lockdep_assert_held(locked_obj->dev_locked);
...
static void try_wake_any_sem(struct ntsync_obj *sem)
{
	...
	lockdep_assert_held(sem->type == NTSYNC_TYPE_SEM);
...
static void try_wake_any_mutex(struct ntsync_obj *mutex)
{
	...
	lockdep_assert_held(mutex->type == NTSYNC_TYPE_MUTEX);
...
static void try_wake_any_event(struct ntsync_obj *event)
{
	...
	lockdep_assert_held(event->type == NTSYNC_TYPE_EVENT);
